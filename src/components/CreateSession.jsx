...
const CreateSession = () => {
  ...
  const handleSubmit = async (status) => {
    try {
      ...
      const res = await axios.post(
        `http://localhost:5000/my-sessions/${status === 'draft' ? 'save-draft' : 'publish'}`,
        { ...form, status },
        {
          headers: {
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-violet-50 via-indigo-50 to-cyan-50 p-4 relative overflow-hidden">
